    ...
    <div className={localStyles.mainContent}>
      <section className={localStyles.footer}>
          ...
          />
              ...
              Empresa dedicada al sector inmobiliario, permite rentar y comprar inmuebles en España, Italia y Portugal.  
              </span>
            </div>
            <div className={localStyles.footerSocial}>
              ...
              <div>
                {socialIcons.map((icon)=>(
                  <Link  key={icon.id} href={icon.link}  onMouseOver={()=>MouseOver(icon.id)}
                  onMouseOut={()=> MouseOut()} >
                     <FontAwesomeIcon style={{color:ishover === icon.id ? icon.color : '#8E8F8C'}} icon={icon.icon} className={localStyles.footerSocialLink}
                     ></FontAwesomeIcon>
                  </Link>
                ))}
              </div>
